...
class _EmergencyPageState extends State<EmergencyPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (resp.statusCode == 200 && resp.data['status'] == true) {
        ...
        _address = (addr is String && addr.isNotEmpty)
            ? addr
  ...
  Widget build(BuildContext context) {
    ...
    final nums = _numbers!;
    final actions = [
      _ActionItem(icon:Icons.local_police,   label:'Police',   onTap:()=>_dial(nums.police)),
      _ActionItem(icon:Icons.local_fire_department, label:'Fire',     onTap:()=>_dial(nums.fire)),
      _ActionItem(icon:Icons.local_hospital, label:'Hospital', onTap:()=>_dial(nums.hospital)),
      _ActionItem(icon:Icons.emergency,      label:'Ambulance',onTap:()=>_dial(nums.ambulance)),
    ];
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children:[
          Row(children:[
            ...
            Text(_address,
                style:TextStyle(fontSize:secondary(), fontWeight:FontWeight.bold)),
          ]),
          const SizedBox(height:25),
          GridView.count(
              crossAxisCount:2, crossAxisSpacing:15, mainAxisSpacing:16,
              childAspectRatio:1.9, shrinkWrap:true, physics:NeverScrollableScrollPhysics(),
              children:actions
          ),
          ...
          Container(
            ...
            child: Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
              Text('Emergency Contacts',
                  style:TextStyle(fontWeight:FontWeight.bold, fontSize:secondary(), color:AppColors.gold)
              ),
              ...
              ListView.builder(
                  ...
                  itemBuilder:(ctx,i){
                    ...
                    return ListTile(
                      ...
                      title:Text(c.name, style:TextStyle(fontSize:secondary(),fontWeight:FontWeight.w600,color:AppColors.green)),
                      subtitle:Text(c.relation, style:TextStyle(fontSize:tertiary(),color:Colors.grey[700])),
                      trailing:CircleAvatar(
                        backgroundColor:AppColors.gold, radius:20,
                        child:IconButton(
...
class _ActionItem extends StatelessWidget {
  ...
  const _ActionItem({required this.icon,required this.label,required this.onTap,Key? key}):super(key:key);
    ...
    return GestureDetector(
        ...
        child:Container(
            ...
            child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
              ...
              Text(label,textAlign:TextAlign.center,style:TextStyle(color:Colors.white,fontSize:tertiary(),fontWeight:FontWeight.w600)),
            ])
        )
    );
  }
